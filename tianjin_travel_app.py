...
# 景点图片字典
ATTRACTION_IMAGES = {
    "天津之眼": "images/tianjin_eye.jpg",
    "意式风情区": "images/italian_style_street.jpg",
    "古文化街": "images/ancient_culture_street.jpg",
    "五大道": "images/five_avenues.jpg",
    "瓷房子": "images/porcelain_house.jpg",
    "天津科技馆": "images/science_museum.jpg",
}

def display_recommendation_with_images(recommendation_text):
    """解析推荐文本并添加景点图片"""
    paragraphs = recommendation_text.split('\n')
    
    for paragraph in paragraphs:
        st.markdown(paragraph)
        for attraction, image_path in ATTRACTION_IMAGES.items():
            if attraction in paragraph:
                try:
                    image = Image.open(image_path)
                    st.image(image, caption=attraction, use_container_width=True)  # 更新这一行
                except:
                    st.warning(f"无法加载{attraction}的图片")
...
def get_recommendations_from_api(user_features):
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
                {"role": "system", "content": "你是一个专业的天津旅游推荐助手，基于用户特征提供个性化的景点推荐，推荐的景点侧重展现天津的文化。"},
...
with st.form("user_input_form"):
    ...
    if submitted:
        ...
        display_recommendation_with_images(recommendation)
        st.success("推荐生成完成！")
